...
describe("lockfile real coverage trigger", function()
  it("calls a real function with no stubs", function()
    local lf = require("src/utils/lockfile")
    local tbl = lf.generate_lockfile_table({ bar = { hash = "zzz" } })
    assert(tbl.package.bar.hash == "zzz")
  end)
end)
